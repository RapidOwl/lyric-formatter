export class TextFormatter {
  constructor() {
    ...
    this.state = new Proxy(typeof defaultState !== 'undefined' ? defaultState : {}, {
      ...
      set: (state, key, value) => {
        ...
        state[key] = value; // Fire callbacks for all state subscribers.
    ...
    this.subscribers = [this.changeSourceText, this.changeFontFamily]; // Set up event handlers that manipulate the state.
  ...
  recipeChanged(sourceText, fontFamily) {
    ...
    textLines.forEach(line => {
      var text;
      var paragraph = document.createElement('p'); // console.log(
      ...
      if (line === '') {
        ...
        paragraph.classList.add('line'); // We add the font family to the measurer as well so it can measure correctly.
        ...
        while (maxTries <= 10 && this.$measurer.offsetWidth <= this.$formattedText.offsetWidth) {
          // This lives inside the loop or it'll never get started.
          this.$measurer.appendChild(text);
          currentFontSize = paragraph.style.fontSize; // console.log(currentFontSize);

          var newFontSize = parseInt(currentFontSize.replace('px', '')) + 8 + 'px'; //console.log(`GUESSED font size: ${newFontSize}`);

          this.$measurer.style.fontSize = newFontSize; // console.log(
